/* Gaudy Oktoberfest illustrations — flat poster SVGs, navy ink outlines.
   Exports React components to window. */
const INK = '#16243f';

function Stein({ size = 64, style, className, rot = 0 }) {
  return (
...
function Pretzel({ size = 64, style, className, rot = 0 }) {
  return (
    <svg viewBox="0 0 64 64" width={size} height={size} className={className}
    style={{ transform: rot ? `rotate(${rot}deg)` : undefined, ...style }}>
      <g fill="none" strokeLinecap="round" strokeLinejoin="round">
        {/* dark outline underlayer */}
...
function Sausage({ size = 64, style, className, rot = 0 }) {
  return (
    <svg viewBox="0 0 64 64" width={size} height={size} className={className}
    style={{ transform: rot ? `rotate(${rot}deg)` : undefined, ...style }}>
      <g transform="rotate(-32 32 32)">
        ...
        {/* mustard zigzag */}
        <path d="M8 33 l5 -3 l5 3 l5 -3 l5 3 l5 -3 l5 3 l5 -3 l5 3" fill="none" stroke="#f4c400" strokeWidth="2.6" strokeLinecap="round" opacity=".0" />
      </g>
    </svg>);

}

function Ball({ size = 40, style, className, color = '#16243f' }) {
  return (
    <svg viewBox="0 0 40 40" width={size} height={size} className={className} style={style}>
      <circle cx="20" cy="20" r="16" fill="none" stroke={color} strokeWidth="3.2" data-comment-anchor="be2b93eba9-path-82-7" />
...
// A hanging garland of food, strung on a wavy rope.
function Garland({ items }) {
  const list = items || ['pretzel', 'stein', 'sausage', 'stein', 'pretzel', 'sausage', 'pretzel', 'stein', 'sausage', 'stein', 'pretzel', 'sausage'];
  const C = { pretzel: Pretzel, stein: Stein, sausage: Sausage };
  return (
    <div className="garland">
      {list.map((k, i) => {
        const Icon = C[k];
        return (
          <span key={i} className="garland__hang" style={{ '--i': i }}>
...
Object.assign(window, { Stein, Pretzel, Sausage, Ball, Garland });
